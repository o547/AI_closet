...
class CoordinateRegisterView(View):
    def get(self,request):
        form=CoordinateForm()
        garments=garment.objects.all()
        return render(request,"closet/coordinate_register.html",{"form":form,"garments":garments})
    def post(self,request):
...
class CoordinateListView(View):
    def get(self,request):
        ...
        return render(request,"closet/coordinate_list.html",{"coordinates":coordinates})
...
class AIEvaluationView(View):
    def get(self,request,id):
        ...
        if evaluated_coordinate.tops:
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+str(evaluated_coordinate.tops)
        if evaluated_coordinate.bottoms:
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.bottoms)
        if evaluated_coordinate.outer != "なし":
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.outer)
        if evaluated_coordinate.inner != "なし":
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.inner)
        if evaluated_coordinate.accessory != "なし":
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.accessory)
        if evaluated_coordinate.additional1 != "なし":
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.additional1)
        if evaluated_coordinate.additional2 != "なし":
            evaluated_coordinate_coordinate=evaluated_coordinate_coordinate+" × "+str(evaluated_coordinate.additional2)
        ...
        return render(request,"closet/ai_evaluation.html",{"response":response,"evaluated_coordinate_coordinate":evaluated_coordinate_coordinate})
...
class AICoordinateView(View):
    def get(self,request):
        ...
        response=cliant.models.generate_content(model="gemini-2.0-flash-lite",contents=prompt)
        response=str(response.text)
        return render(request,"closet/garment_list.html",{"garments":garments,"response":response})
